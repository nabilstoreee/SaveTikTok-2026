...
import { BrandAd, MOCK_VIDEO_ADS, getAdsFromFirestore } from '../lib/adsDb';
export type { BrandAd };
export { MOCK_VIDEO_ADS };
...
export function VideoAdModal({ isOpen, onClose, onAdCompleted }: VideoAdModalProps) {
  ...
  // Initialize random ad from Firestore
  useEffect(() => {
    if (isOpen) {
      const loadAdAsync = async () => {
        try {
          const adsList = await getAdsFromFirestore();
