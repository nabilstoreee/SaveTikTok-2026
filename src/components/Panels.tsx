...
import { loadUser, ActivityLog, getRegisteredUsers, saveRegisteredUsers, getBannedEmails, saveBannedEmails, getBannedIps, saveBannedIps, BlacklistItem, getSimulatedIp, BAN_DURATIONS } from '../lib/user';
import { BrandAd, MOCK_VIDEO_ADS } from './VideoAdModal';
...
export function AdminPanel({ isOpen, onClose, settings, onSave }: { isOpen: boolean; onClose: () => void, settings: SiteSettings, onSave: (s: SiteSettings) => void }) {
  ...
  // Load custom ads dynamically when AdminPanel is opened
  useEffect(() => {
    if (isOpen) {
      try {
        ...
        if (stored) {
          ...
          // Pre-populate with default MOCK_VIDEO_ADS so they can be modified by the admin
          setCustomAds(MOCK_VIDEO_ADS);
          localStorage.setItem('savetik_custom_ads', JSON.stringify(MOCK_VIDEO_ADS));
        }
      } catch (e) {
        console.error("Failed to parse custom ads from localStorage", e);
      }
    }
  }, [isOpen]);

  const handleSaveAdsToStorage = (updatedAds: BrandAd[]) => {
    setCustomAds(updatedAds);
    localStorage.setItem('savetik_custom_ads', JSON.stringify(updatedAds));
  };
  ...
  const handleApprovePayment = (req: any, durationDays: number) => {
    ...
    localStorage.setItem('savetik_payment_requests', JSON.stringify(updatedRequests));
  ...
  const handleRejectPayment = (req: any) => {
    const payRaw = localStorage.getItem('savetik_payment_requests');
    let currentRequests = paymentRequests;
    if (payRaw) {
      try {
  ...
  const handleSendAdminChatMessage = (reqId: string) => {
  ...
  const handleDeleteAdminChatMessage = (reqId: string, msgId: string) => {
    const payRaw = localStorage.getItem('savetik_payment_requests');
    let currentRequests = paymentRequests;
    if (payRaw) {
      try {
