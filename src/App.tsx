...
import { UserState, loadUser, saveUser, isPremiumActive, checkBanStatus, getSimulatedIp, getBannedEmails, getBannedIps, saveBannedEmails, saveBannedIps } from "./lib/user";
...
export default function App() {
  ...
  const handleDownload = async () => {
    ...
    // Rate Limiter Check for non-admin
    if (settings.rateLimitEnabled && user.email !== 'jrnabil570@gmail.com') {
      const trackerKey = `savetik_download_timestamps_${user.email || 'anonymous'}`;
      const now = Date.now();
      ...
      if (recentTimestamps.length >= settings.rateLimitMaxRequests) {
        if (settings.autoBlockSuspiciousIps) {
          // Block email
          if (user.email) {
            const bannedEmails = getBannedEmails();
            if (!bannedEmails.some(b => b.value.toLowerCase() === user.email!.toLowerCase())) {
              bannedEmails.push({
                value: user.email,
                reason: `Sistem Otomatis: Terdeteksi aktivitas bot spam (Melebihi batas ${settings.rateLimitMaxRequests} unduhan per ${settings.rateLimitWindowMinutes} menit).`,
          ...
          // Block IP
          const userIp = user.ip || getSimulatedIp(user.email || 'anon');
          if (userIp && userIp !== '127.0.0.1') {
            const bannedIps = getBannedIps();
            if (!bannedIps.some(b => b.value === userIp)) {
              bannedIps.push({
                value: userIp,
                reason: `Sistem Otomatis: Terdeteksi aktivitas bot spam dari IP ini (Melebihi batas ${settings.rateLimitMaxRequests} unduhan per ${settings.rateLimitWindowMinutes} menit).`,
                createdAt: new Date().toLocaleString()
              });
          ...
          setError('Akses Diblokir! Aktivitas Anda terdeteksi sebagai spamming otomatis (bot). IP dan akun Anda telah didaftarkan ke Daftar Hitam Keamanan.');
          setTimeout(() => {
        ...
        setError(`Batas Unduhan Terlampaui! Anda hanya diperbolehkan melakukan maksimal ${settings.rateLimitMaxRequests} unduhan per ${settings.rateLimitWindowMinutes} menit untuk mencegah aktivitas spam.`);
        return;
      }
      ...
      try {
        localStorage.setItem(trackerKey, JSON.stringify(recentTimestamps));
      } catch (e) {}
    }
  ...
  const isBannedMessage = checkBanStatus(user);
  if (isBannedMessage && user.email?.toLowerCase() !== 'jrnabil570@gmail.com') {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0b0f19] flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-8 max-w-md w-full text-center shadow-xl relative">
          <div className="w-20 h-20 bg-rose-500/10 text-rose-500 mx-auto rounded-[1.5rem] flex items-center justify-center mb-6">
            <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Akses Ditangguhkan</h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
            {isBannedMessage}
          </p>
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/50 dark:border-slate-800/80 text-left mb-6 text-xs space-y-2">
            <div className="flex justify-between"><span className="text-slate-400">Username:</span> <span className="font-bold text-slate-700 dark:text-slate-200">{user.username}</span></div>
            <div className="flex justify-between"><span className="text-slate-400">Email:</span> <span className="font-bold text-slate-700 dark:text-slate-200">{user.email || '-'}</span></div>
            <div className="flex justify-between"><span className="text-slate-400">IP Address:</span> <span className="font-mono text-indigo-500 font-bold">{user.ip || getSimulatedIp(user.email || '')}</span></div>
          </div>
          <div className="flex justify-center gap-3">
            ...
            <a 
              ...
              className="px-6 py-2.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md transition-all"
            >
              Hubungi Dukungan
            </a>
          </div>
