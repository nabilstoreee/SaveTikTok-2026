...
export async function fetchTikTok(url: string): Promise<VideoResultData> {
  ...
  return {
    ...
    authorAvatar: data.data.author?.avatar || data.data.author?.avatar_thumb || data.data.author?.avatar_medium,
    authorNickname: data.data.author?.nickname || data.data.author?.unique_id,
    authorStats: {
      followers: data.data.author?.followers || data.data.author?.follower_count || undefined,
      totalLikes: data.data.author?.heart || data.data.author?.total_favorited || data.data.author?.digg || data.data.digg_count,
      videosCount: data.data.author?.video_count || undefined
    },
...
export async function fetchYouTube(url: string, quality?: string): Promise<VideoResultData> {
  ...
  return {
    ...
    author: data.author || data.channel || 'YouTube Creator',
    authorNickname: data.author || data.channel || 'YouTube Creator',
    authorStats: {
      followers: data.subscribers || undefined,
      totalLikes: data.likes || undefined
    },
    platform: 'youtube',
